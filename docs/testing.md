...
in [`smoke/`](smoke/) — **`npm run smoke`**, 674 assertions across ten files. They
...
`xcel-nav-collapse.html`, `xcel-study-pace-readiness.html`,
`xcel-pace-presets.html` and `xcel-home.html` — each ship their own suite:
being row-less is not a reason to go uncovered, it is only the reason
