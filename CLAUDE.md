...
### No cart, no member upsells (2026-09-21)

The direct ask. Two removals, **different mechanisms**, and the difference is
the point — it is the `profile-motivational-statement` / Membership Plan split
applied again:

| Removed | Mechanism | Archive row |
|---|---|---|
| The membership upsell band | gated on `supportsMembership` — CORRECTNESS | **no** |
| The Career Tools "Member Exclusive" badge + lede | gated on `supportsMembership` — CORRECTNESS | **no** |
| The header cart | unwired — EDITORIAL | **yes** (`header-cart`) |

A brand predicate returns its surface on its own for a brand that has one, so a
row would tell the next person to re-add something the predicate is deliberately
withholding. The cart has no predicate to hang on, and inventing `supportsCart`
would be one that is false for every member of a one-brand union.

**The upsell band is guarded INSIDE the component, not at the call sites.**
There are five (`MembershipOverview` twice, `UpgradeBanner`,
`MembershipBenefitsPanel`, `MembershipStandalonePage`) and gating four of five
is how this defect survives a sweep.

**Why the band's existing `access === 'full'` guard did not already cover it**,
which is the part not to re-derive. `supportsMembership`'s own note says a brand
here must have a SINGLE `high` tier so `accessForTier` resolves to `'full'` —
and it does, **until something sets the tier to `'non-member'`**, which the demo
tier switch and the stored account state both can. That is the exact "SUPPRESS,
NEVER DOWNGRADE" failure that note warns about; it lists four surfaces the
downgrade breaks, and this band was a fifth.

**The symptom was on screen and easy to read past:** the title rendered
*"Unlock Membership Benefits. Starting at&nbsp;&nbsp;/ year"* — with NOTHING
between "at" and "/", because `membershipUpgradeFixtures` deliberately leaves
XCEL's price blank ("a filled-in stub here would read as product truth in a
screenshot"). **That file's comment claimed "every surface reading this map is
suppressed for it". It was not true of this one**, and is now.

**The Career Tools gate takes the WHOLE section**, not just the badge and the
lede. `benefitRowsFor` returns `[]` for every brand but Elite, so on XCEL
`BenefitSections` self-hides and what was left was a section header over nothing
— the "reads as a load failure" defect `dashboard-recommended` states as its own
reason for removing the header along with the cards. This is the bug the flag
audit recorded and left ("**not fixed here**"); it is fixed now, and it is the
**fourth** instance of one root cause — after the rail's Membership link, the
Membership Plan card and the Account Details "Member" pill. The rule that section
states holds: **when a brand predicate turns something off, sweep for the places
that ask the TIER instead**, because XCEL's only tier is `high` and every
`isMember` check passes for it.

**The cart was a `<Link to="#">`** — a control that had never gone anywhere, in
a product where the learner is already enrolled and buys course packages on
xcelsolutions.com rather than in the LMS. `CartButton` is kept and **exported**
(an unreferenced local function is a lint error — the `NavProfileHeader`
precedent). The restore note names the dead `to="#"` as the thing deliberately
NOT restored: a restore that matters wants a real destination.

**The sweep was empirical, not inferred.** All 13 rail sections were walked in
the browser with `dashboard-career-tools` forced ON and the tier at
`non-member` — the worst case — and checked for cart, upsell-band, "Member
Exclusive" and "included with membership" copy. All clean.

#### A test passed for the wrong reason, twice, in one file

Worth recording because both are the same shape:

- The first cart assertion rendered `PlatformShell` and checked
  `queryByLabelText('Cart')` was null. **It passed before the change was made**
  — `PlatformShell` does not render the Header at all; `AppLayout` does, above
  the `<Outlet/>`. The absence was measuring a component that was never in the
  tree.
- Replacing it with a source read via `new URL(…, import.meta.url)` failed the
  whole file at collection: under jsdom that `URL` is jsdom's and Node's
  `readFileSync` rejects it. **CLAUDE.md already records this exact trap**
  taking out `UxDashboard.smoke.test.tsx`. A plain relative path, like the two
  sibling reads in `HeaderAccountMenu.test.tsx`.

The DOM proof ended up in **`Header.test.tsx`**, which already mounts the real
Header: its `getByRole('link', {name: /cart/i})` was **inverted, not deleted** —
the same treatment the Membership link two lines above it already had, and for
the same reason. `NoCartNoMemberUpsell.test.tsx` keeps the archive-convention
half. Every removal assertion was verified to fail with the change reverted.
